...
v_train_path = "v_train_data_spectral.bson";
c_train_path = "c_train_data_spectral.bson";
...
model_name = "gaussian_model_spectral";
# Path of potential loaded model. 
load_path = nothing; # "trained_models/gaussian_model_spectral.bson";  # Set to `nothing` to initialize instead
...
ubar_test = v_train[:,:,:,1]; 
ubar_test = reshape(ubar_test, (2*params_les.K), (2*params_les.K), 2, batch_size) |> dev; # size: (128,128,2,1) - GPU (if we generate random) or CPU (if we take the initial condition for the training)
u_les = ubar_test;
...
for i = 1:nt+1
    ...
    if i > 1
        global t
        u_les = step_rk4(CuArray(ubar_test), params_les, dt) |> dev
        closure = generate_closure(velocity_cnn, ps_drift, _st_drift, v_train_standardized[:,:,:,i], num_steps, batch_size, dev; method=:euler) |> dev; # size: (128,128,2,1) 
