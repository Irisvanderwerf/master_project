...
for i in 1:size(c_train, 4)
    predicted_c_standardized = generate_closure(velocity_cnn, ps_drift, st_drift, v_train_standardized[:,:,:,i], num_steps, 1, dev; method=:euler);
    predicted_c = inverse_standardize_per_channel(predicted_c_standardized, closure_means, closure_std);
    difference = mean(abs2, c_train[:, :, :, i] - predicted_c);

    # Update the maximum difference
    max_difference = max(max_difference, difference);
    min_difference = min(min_difference, difference);
end
println("The maximum mean squared error in the training set: ", max_closure_MSE)
println("The minimum mean squared error in the training set: ", min_closure_MSE)

### Time evolution where we start with filtered DNS solution ###
# Physical time 
t = 0.0f0; # Initial time t_0
dt = 2.0f-4; # Time step Δt
nt = 4; # Number of time steps (number of training and test samples)
...
for i = 1:nt+1
    ...
    if i > 1
        ...
        closure = generate_closure(velocity_cnn, ps_drift, st_drift, v_train_standardized[:,:,:,i], num_steps, batch_size, dev; method=:euler) |> dev; 
