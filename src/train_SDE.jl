...
function initialize_or_load_model(model_name::String, is_gaussian::Bool, network::Any, dev, load_path::Union{String, Nothing} = nothing)
    if isnothing(load_path)
        ...
        opt_drift = Optimisers.setup(Adam(1.0e-4, (0.9f0, 0.99f0), 1.0e-6), ps_drift)

        if !is_gaussian
            # Denoiser term initialization
            ps_denoiser, st_denoiser = Lux.setup(Random.default_rng(), network) |> dev
            opt_denoiser = Optimisers.setup(Adam(1.0e-4, (0.9f0, 0.99f0), 1.0e-6), ps_denoiser)
        else
        ...
        if !is_gaussian
            ...
            ps_drift, st_drift, opt_drift, ps_denoiser, st_denoiser, opt_denoiser = load_model(load_path, is_gaussian; dev)
        else
            println("Loading models from path: $load_path")
            ps_drift, st_drift, opt_drift = load_model(load_path, is_gaussian; dev)
        end
    end
...
function load_model(file_path, is_gaussian; dev)
