...
function initialize_or_load_model(model_name::String, network::Any, load_path::Union{String, Nothing} = nothing; dev)
    ...
    if isnothing(load_path)
        ...
        opt_drift = Optimisers.setup(Adam(1.0e-4, (0.9f0, 0.99f0), 1e-8), ps_drift)
    else
...
function get_minibatch_NS(images, batch_size, batch_index; shuffle=true, shuffled_indices=nothing)
    start_index = (batch_index - 1) * batch_size + 1
    end_index = min(batch_index * batch_size, size(images, 4))
    if shuffle
        @assert shuffled_indices !== nothing "Error: indices must be provided when shuffle=true"
...
function loss_fn_det(velocity, dI_dt_sample)
    loss = sum(abs2, velocity .- dI_dt_sample) ./ sum(abs2, dI_dt_sample)
    return loss
end
...
function train!(train_data, val_data, batch_size, num_epochs, ps_drift, st_drift, opt_drift, velocity_cnn, save_path, model_name, num_samples, load_path, ϵ; dev=gpu.device())
    ...
    for epoch in start_epoch:num_epochs
        if !stop_training 
            println("Epoch $epoch")
            shuffled_indices = shuffle(1:(size(train_images, 4)))
            ...
            for batch_index in 1:num_batches-1
                initial_sample = Float32.(get_minibatch_NS(initial_train_images, batch_size, batch_index; shuffle=true, shuffled_indices=shuffled_indices)) |> dev
                target_sample = Float32.(get_minibatch_NS(train_images, batch_size, batch_index; shuffle=true, shuffled_indices=shuffled_indices)) |> dev  
                target_labels_closure_sample = Float32.(get_minibatch_NS(train_labels_closure, batch_size, batch_index; shuffle=true, shuffled_indices=shuffled_indices)) |> dev 
                target_labels_state_sample = Float32.(get_minibatch_NS(train_labels_state, batch_size, batch_index; shuffle=true, shuffled_indices=shuffled_indices)) |> dev 
                ...
                I_sample = Float32.(stochastic_interpolant(initial_sample, target_sample, W_sample, t_sample, ϵ)) |> dev
                loss_drift_closure = (ps_) -> begin
                    dI_dt_sample = Float32.(time_derivative_stochastic_interpolant(initial_sample, target_sample, W_sample, t_sample, ϵ))
            ...
            for test_batch_index in 1:num_test_batches
                test_initial_sample = Float32.(get_minibatch_NS(initial_test, batch_size, test_batch_index; shuffle=false, shuffled_indices=nothing)) |> dev
                test_target_sample = Float32.(get_minibatch_NS(target_test, batch_size, test_batch_index; shuffle=false, shuffled_indices=nothing)) |> dev 
                test_target_label_closure_sample = Float32.(get_minibatch_NS(target_label_closure_test, batch_size, test_batch_index; shuffle=false, shuffled_indices=nothing)) |> dev
                test_target_label_state_sample = Float32.(get_minibatch_NS(target_label_state_test, batch_size, test_batch_index; shuffle=false, shuffled_indices=nothing)) |> dev
                ...
                test_I_sample = Float32.(stochastic_interpolant(test_initial_sample, test_target_sample, test_W_sample, test_t_sample, ϵ)) |> dev
                test_dI_dt_sample = Float32.(time_derivative_stochastic_interpolant(test_initial_sample, test_target_sample, test_W_sample, test_t_sample, ϵ)) |> dev
...
function train_deterministic!(train_data, val_data, batch_size, num_epochs, ps_drift, st_drift, opt_drift, velocity_cnn, save_path, model_name, num_samples, load_path; dev=gpu.device())
    train_images, train_labels_state = train_data.target, train_data.state |> dev;
    target_test, target_label_state_test = val_data.target, val_data.state |> dev;

    max_time_steps = size(train_images, 4)
    if isnothing(num_samples) || num_samples > max_time_steps
        num_samples = max_time_steps 
    end
    ...
    for epoch in start_epoch:num_epochs
        if !stop_training 
            ...
            for batch_index in 1:num_batches-1
                ...
                loss_drift_closure = (ps_) -> begin
                    closure, st_drift = Lux.apply(velocity_cnn, (target_labels_state_sample), ps_, st_drift)
                    return loss_fn_det(closure, target_sample), st_drift
                end
            ...
            for test_batch_index in 1:num_test_batches
                ...
                test_closure, _ = Lux.apply(velocity_cnn, (test_target_label_state_sample), ps_drift, _st_drift)
                test_drift_loss += loss_fn_det(test_closure, test_target_sample)
            end
            test_drift_loss /= num_test_batches
            push!(test_drift_losses, test_drift_loss)
            println("Validation loss for drift term: $test_drift_loss")
