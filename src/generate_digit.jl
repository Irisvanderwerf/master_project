...
function forward_euler(velocity_cnn, ps, st, images, t, dt, batch_size, dev)
...
function runge_kutta_4(velocity_cnn, ps, st, images, t, dt, batch_size, dev)
    # Reshape t_sample to match the batch size
    t_sample = Float32.(fill(t, (1, 1, 1, batch_size))) |> dev
    ...
    velocity_k2, st = Lux.apply(velocity_cnn, (images .+ k1 ./ 2, t_sample_next), ps, st)
    k2 = dt .* velocity_k2

    t_sample_next = Float32.(fill(t + dt/2, (1, 1, 1, batch_size))) |> dev
    velocity_k3, st = Lux.apply(velocity_cnn, (images .+ k2 ./ 2, t_sample_next), ps, st)
    k3 = dt .* velocity_k3

    t_sample_next = Float32.(fill(t + dt, (1, 1, 1, batch_size))) |> dev
    velocity_k4, st = Lux.apply(velocity_cnn, (images .+ k3, t_sample_next), ps, st)
    k4 = dt .* velocity_k4
...
function generate_digit(velocity_cnn, ps, st, initial_gaussian_image, num_steps, batch_size, dev; method=:rk4)
    ...
    for i in 1:num_steps-1
        ...
        if method == :euler
            images, st = forward_euler(velocity_cnn, ps, st, images, t, dt, batch_size, dev)
        elseif method == :rk4
            images, st = runge_kutta_4(velocity_cnn, ps, st, images, t, dt, batch_size, dev)
        else
